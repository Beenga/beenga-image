...
// "an indian man" and "an indian woman" at three different seeds all came back
// in the same light-blue shirt. Everything describing the PERSON was identical
// across seeds — only the scene line varied — so the model had nothing to vary
// on and collapsed to a single archetype.
//
// sari-variety and scene-variety already solve this with a per-seed pick; the
// modern-dress default was simply never wired into the same machinery.
const MODERN_DRESS = "Modern everyday clothing.";   // fallback when sex is unclear
const MENS_DRESS = [
...
// sunlight/daylight/sunlit are spelled out: `light` cannot match inside a
// compound (no word boundary in "sunlight"), so "Natural morning sunlight"
// was getting "Bright natural daylight, clear colour" appended on top of it.
// "moonlight" and "night" were listed and worked; their daytime counterparts
// were simply missed. Not `sun\w*` — that matches sunday and sunflower.
const LIGHTING = /\b(sunlight|sunlit|daylight|sunshine|sunny|light|lighting|lit|sunset|sunrise|golden\s+hour|night|dusk|dawn|evening|twilight|midnight|moonlit|moonlight|firelight|lantern|candlelit|shade|shadow|backlit|neon|candle|lamp|studio|overcast|cloudy|moody|dim|gloom|dark|silhouette)\w*\b/i;
...
// Per-seed variants of the house look. Same job as HOUSE_LOOK — attractive by
// default, which is the documented house decision — but varying the face so
// three seeds do not return the same person.
//
// These DO carry a complexion, which reverses the earlier "no complexion in the
// house look" rule, and that was a deliberate request: the flat no-complexion
// default produced one repeated face. The answer to "always the same girl" and
// to "bring back fair" is the same thing — a RANGE. A fixed fair default would
// make the sameness worse, being one more constant pinning every face to one
// look. The range spans fair to deep so fair appears often and never always.
//
// Still fully gated: if the caller states a complexion, or the prompt names a
// minor, house-look does not fire at all and none of this is emitted.
const HOUSE_LOOKS = [
...
export function enhance(raw, { contemporary = true, reinforce = true, variant = "",
  ...
  if (contemporary && INDIA.test(raw) && !TRADITIONAL_INTENT.test(raw)) {
    ...
    if (!GARMENT.test(raw) && PERSON.test(raw)) {
      // Per-seed, so re-rolling varies the outfit instead of returning the same
      // light-blue shirt every time. Falls back to the generic line when the
      // prompt does not indicate sex.
      const male = MALE.test(raw) && !FEMALE_ONLY.test(raw);
      const female = FEMALE_ONLY.test(raw) || /\b(woman|women|girl|lady|ladies|her|she)\b/i.test(raw);
      const list = male ? MENS_DRESS : female ? WOMENS_DRESS : null;
      add("modern-dress-default",
          list ? `Wearing ${list[hash(raw + variant) % list.length]}.` : MODERN_DRESS);
  ...
  if (PERSON.test(raw) && !FOREIGN.test(raw) && !TRADITIONAL_INTENT.test(raw) &&
    ...
    if (faceIsSubject(raw)) {
      add("house-look",
          region + HOUSE_LOOKS[hash(raw + variant) % HOUSE_LOOKS.length] + "." + maleTail);
      ...
      const terse = " " + APPEARANCE_TERSE[hash(raw + variant) % APPEARANCE_TERSE.length];
      const identity = (region ? HOUSE_REGION_ALONE : "") + terse + maleTail;
      if (identity) {
