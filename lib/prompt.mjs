...
const FRAGILE = [
  ...
  { test: (raw) =>
      PERSON.test(raw) && INDIA.test(raw) &&
      !/\bindian\b|\bsouth\s+asian\b|\bdesi\b/i.test(raw) &&
      !/\b(european|caucasian|white|african|black|chinese|japanese|korean|thai|arab|latina?|hispanic|russian|american|british)\b/i.test(raw),
